...
const INITIAL_URL_GROUPS: URLGroup[] = [
  { id: 'gemini-overview', name: 'Gemini Docs Overview', urls: GEMINI_DOCS_URLS, documents: [] },
  { id: 'model-capabilities', name: 'Model Capabilities', urls: MODEL_CAPABILITIES_URLS, documents: [] },
];

const App: React.FC = () => {
  const [urlGroups, setUrlGroups] = useState<URLGroup[]>(() => {
    const saved = localStorage.getItem('knowledgeBaseGroups');
    if (saved) {
      try {
        return JSON.parse(saved);
      } catch (e) {
        console.error("Failed to parse saved knowledge base groups", e);
      }
    }
    return INITIAL_URL_GROUPS;
  });
  const [activeUrlGroupId, setActiveUrlGroupId] = useState<string>(urlGroups[0]?.id || INITIAL_URL_GROUPS[0].id);
  ...
  const handleRemoveDocument = (id: string) => {
    setUrlGroups(prevGroups =>
      prevGroups.map(group => {
        if (group.id === activeUrlGroupId) {
          return { ...group, documents: group.documents.filter(doc => doc.id !== id) };
        }
        return group;
      })
    );
  ...
  const handleSendMessage = async (query: string) => {
    ...
    try {
      const response = await generateContentWithUrlContext(query, currentUrlsForChat, activeGroup?.documents || []);
      
      if (response.functionCalls && response.functionCalls.length > 0) {
        // Handle function calls
        const functionCall = response.functionCalls[0];
        if (functionCall.name === 'addUrl') {
            handleAddUrl(functionCall.args.url as string);
            setChatMessages(prevMessages => [...prevMessages, { id: `system-${Date.now()}`, text: `Added URL: ${functionCall.args.url}`, sender: MessageSender.SYSTEM, timestamp: new Date() }]);
        } else if (functionCall.name === 'removeUrl') {
            handleRemoveUrl(functionCall.args.url as string);
            setChatMessages(prevMessages => [...prevMessages, { id: `system-${Date.now()}`, text: `Removed URL: ${functionCall.args.url}`, sender: MessageSender.SYSTEM, timestamp: new Date() }]);
        } else if (functionCall.name === 'listKnowledgeBase') {
            const listText = `Current URLs: ${currentUrlsForChat.join(', ')}\n\nCurrent Documents: ${activeGroup?.documents.map(d => d.name).join(', ') || 'None'}`;
            setChatMessages(prevMessages => [...prevMessages, { id: `system-${Date.now()}`, text: listText, sender: MessageSender.SYSTEM, timestamp: new Date() }]);
        } else if (functionCall.name === 'saveMemory') {
            const title = functionCall.args.title as string || 'Memory';
            const content = functionCall.args.content as string;
            handleAddDocument({ id: Date.now().toString(), name: title, content });
            setChatMessages(prevMessages => [...prevMessages, { id: `system-${Date.now()}`, text: `Saved memory: "${title}"`, sender: MessageSender.SYSTEM, timestamp: new Date() }]);
        }
        setIsLoading(false);
      } else {
      ...
      <div className="flex h-full w-full md:p-4 md:gap-4">
        ...
        `}>
          <KnowledgeBaseManager
            ...
            documents={activeGroup?.documents || []}
